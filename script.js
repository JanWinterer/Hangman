...
async function onButtonClickInit() {
    try {
        var hangmanStr = await getRandomElementFromCSV(csvParsed);
        targetWord = hangmanStr;
...
function playGame(hangmanStr) {
    document.getElementById("Button1").addEventListener("click", onButtonClickInit);

    function getRandomElementFromCSV(csvParsed) {
    ...
    function handleEnterPress() {
        ...
        if (enteredChars.includes(textBoxValue.value)) {
            ...
            enteredChars.push(textBoxValue);
            handleNewChar(textBoxValue);
        }
    }

    function handleNewChar(textBoxValue) {
        if (!targetWord.includes(textBoxValue)) {
            HP -= 1;
